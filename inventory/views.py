...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='deliver', url_name='deliver-list', permission_classes=[IsAdminUser])
    def deliver_placeholder(self, request):
        return Response({'error': 'Please specify an order ID.'}, status=status.HTTP_400_BAD_REQUEST)
...
class OrderViewSet(viewsets.ModelViewSet):
    """
    Dealers place and manage their own orders.
    Admin can see all orders.

    POST   /api/orders/                  → create draft order
    GET    /api/orders/                  → list orders (dealer sees own, admin sees all)
    GET    /api/orders/{id}/             → order detail
    PUT    /api/orders/{id}/             → update draft order items
    POST   /api/orders/{id}/confirm/    → confirm order (validates + deducts stock)
    ...
    def get_queryset(self):
        user = self.request.user
        qs = Order.objects.select_related('dealer__user').prefetch_related('items__product').all()
        # Dealers only see their own orders
        if not user.is_staff:
            try:
                dealer = user.dealer_profile
                return qs.filter(dealer=dealer)
                ...
                return Order.objects.none()
        # Admin sees all orders with optional filters
        status_filter = self.request.query_params.get('status')
        dealer_filter = self.request.query_params.get('dealer')
        if status_filter:
            qs = qs.filter(status=status_filter)
    ...
    def create(self, request, *args, **kwargs):
        ...
        Dealer creates order for themselves.
        Admin can create order for any dealer (pass dealer_id in body).
        """
        user = request.user

        # Resolve which dealer this order is for
        if user.is_staff:
            dealer_id = request.data.get('dealer_id')
            if not dealer_id:
                return Response(
    ...
    def confirm(self, request, pk=None):
        ...
        1. Validate order can transition to confirmed
        2. Check ALL items have sufficient stock
        3. If ANY item fails → reject entire order with clear error
        4. If ALL pass → deduct stock atomically (all or nothing)
        """
        order = self.get_object()
        ...
        # ── Stock Validation ──────────────────────────────────────────────────
        # Check ALL items before doing anything — collect all errors at once
        stock_errors = []
        items_to_process = []
        ...
        # If ANY item failed validation → reject entire order
        if stock_errors:
            return Response({
                'error': 'Order confirmation failed due to insufficient stock.',
                'stock_errors': stock_errors
        ...
        # All items passed — now deduct stock atomically
        # If anything fails here, ALL changes are rolled back (no partial deductions)
        try:
            with transaction.atomic():
                for item, inventory in items_to_process:
                    new_quantity = inventory.quantity - item.quantity
